               ...
               <tr>
                  ...
                  <td>
                     <ul>
                        ...
                        <a href="{{ route('jenis-pelayanan.show', $jenisPelayanan->uuid) }}" class="btn btn-sm btn-info"><i
                              class="ti-info-alt"></i></a>
                        @endcan

                        @can('permission', 'edit-jenis-pelayanan')
                        <a href="{{ route('jenis-pelayanan.edit', $jenisPelayanan->uuid) }}" class="btn btn-sm btn-warning"><i
                              class="ti-pencil-alt"></i></a>
                        @endcan

                        @can('permission', 'delete-jenis-pelayanan')
                        <button class="btn btn-sm btn-danger" type="button" data-bs-toggle="offcanvas"
                           data-bs-target="#jenisPelayanan-{{ $jenisPelayanan->uuid }}" aria-controls="jenisPelayanan"><i
                              class="ti-trash"></i>
                        </button>

                        <div class="offcanvas offcanvas-end" tabindex="-1" id="jenisPelayanan-{{ $jenisPelayanan->uuid }}"
                           aria-labelledby="jenisPelayananLabel">
                           ...
                           <div class="offcanvas-body p-4">
                              <form action="{{ route('jenis-pelayanan.destroy', $jenisPelayanan->uuid) }}" method="POST">
                                 @csrf
                                 @method('DELETE')
                                 <div class="mb-3">
                                    <span class="fs-6">Hapus {{ $jenisPelayanan->nama_pelayanan . ' | Tahun ' . $jenisPelayanan->tahun }}?</span>
                                 </div>
